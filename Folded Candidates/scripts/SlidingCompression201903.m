...
sim_name = 'a3d';
num_frames_to_skip = 0; % 2000;

structures_root = '../structures/';
data_root = '../data/20190308/';

output_folder = ['../results201903/' sim_name '/'];
time_stamp_string = datestr(now, 'yyyymmdd-hhMMss');

%%
VerySmallNumber = 1e-10;

%%
pdb = pdbread([structures_root sim_name '.pdb']);
...
for n = 0:27
    %n = 2;
    rama = load(sprintf('%s%s/%s.%d.rama.pkl.mat', data_root, sim_name, sim_name, n));
    pe = load(sprintf('%s%s/%s.%d.pe.pkl.mat', data_root, sim_name, sim_name, n));
    pe = pe.data';
    frames = double(rama.data);
    
    % throw out the first 2000 frames
    if (num_frames_to_skip > 0)
        frames(1:num_frames_to_skip, :, :) = [];
        pe(1:num_frames_to_skip) = [];
    end
    ...
    if (numOfStatesOptimizationMode)
        save([time_stamp_string ' states optimization.mat'], 'results');
    else
        save(sprintf('%s[%s] %s_%d sliding window (%d frames, %d stride) %s.mat', output_folder, time_stamp_string, sim_name, n, windowLength, windowStride), 'results');
    end

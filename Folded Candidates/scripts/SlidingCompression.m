...
%%
n = 4;
rama = load(['hyp.' num2str(n) '.rama.pkl.mat']);
...
if (numOfStatesOptimizationMode)
    results = analysis.Analyze(frames(10e3 + [1:5000], :, :));
else
...
if (numOfStatesOptimizationMode)
    ...
    save([time_stamp_string sprintf(' hyp%d sliding window (%d frames, %d stride).mat', n, windowLength, windowStride)], 'results');
end

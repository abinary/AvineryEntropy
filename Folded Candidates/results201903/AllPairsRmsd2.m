function [rmsd, ij] = AllPairsRmsd2(X, Y, frame_idx, dof_idx, coord_idx)
...
if (1)
    X = permute(X, [dof_idx coord_idx frame_idx]); % frame index should be last
...
if (1)
    ...
    T_all = gather(T_all);
    T_all = reshape(T_all, [size(X, frame_idx) size(Y, frame_idx) 3 3]);

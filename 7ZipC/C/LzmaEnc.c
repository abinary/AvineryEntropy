...
unsigned int _maxUnpackedBlockSize = 1 << 17;
void SetMaxUnpackedBlockSize(unsigned int maxUnpackedBlockSize)
{
	_maxUnpackedBlockSize = maxUnpackedBlockSize;
}
...
static SRes LzmaEnc_CodeOneBlock(CLzmaEnc *p, Bool useLimits, UInt32 maxPackSize, UInt32 maxUnpackSize)
{
  ...
  for (;;)
  {
    ...
    if (p->additionalOffset == 0)
    {
      ...
      else if (processed >= _maxUnpackedBlockSize) //(processed >= (1 << 17))
      {
